...
def handleRows(PSSM, SWITCH, COUNT):
	'''
	if SWITCH=0, we filter no elements.
	if SWITCH=1, we filter all the negative elements.
	if SWITCH=2, we filter all the negative elements and positive elements greater than expected.
	'''
	'''
...
def preHandleColumns(PSSM,STEP,PART,ID):
	...
	'''
	'''
	if KEY=0, we don't divide each elements by the sum of elements in its column.
	if KEY=1, we divide each elements by the sum of elements in its column.
	'''
	if PART==0:
		print "PART=",PART
		PSSM=PSSM[:,1:21]
